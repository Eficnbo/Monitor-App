...
Database:
    ...
    Indices are used when joining tables if the queries are such that they are used often, check README for indexing!
...
User interface / views:
    ...
    User interface uses a style library or self-made stylesheets (see e.g. Twitter Bootstrap for a style library):
        Using bootstrap over a content delivery network, check views/partials/header.ejs
